...
exports.MAX_DEPTH = exports.Codec = void 0;
...
const serialize_meta = (str, meta) => {
    ...
    return str + String.fromCodePoint(num_to_code_point(+meta.array), num_to_code_point(depth));
};
...
function encode_array(res, values) {
    ...
    assert(!!fixed_depth || depths.length === values.length, TypeCodecError, 'Wrong depths array length:', depths, values);
...
function decode_array(str, meta) {
    assert(!this.meta || meta.depth > 0 || (meta.depth === 0 && Array.isArray(this.depth)), TypeCodecError, 'Array decoding error (check inputs and codec config)');
    ...
    while (i < length) {
        ...
        while (value_done < depth) {
            ...
            if (symbol_done === SAFE_BITS) {
                ...
                if ((i + 1) === length && value_done === depth)
                    break;
...
class Codec {
    ...
    encode(arg) {
        assert((Array.isArray(arg) || !!arg.BYTES_PER_ELEMENT) === this.array, TypeCodecError, 'Incompatible codec (array <=> single value), arg =', arg);
        let res = '';
    ...
    decode(str, length_out) {
        ...
        assert(meta.array === this.array, TypeCodecError, 'Incompatible codec (array <=> single value), str =', str);
        ...
        let acc = 0, pow = 0;
        const depth = ((_a = meta.depth) !== null && _a !== void 0 ? _a : this.depth);
